...
.PHONY: flake8
flake8: venv ## flake8
	$(VENV_BIN)/flake8 aioapp_redis examples tests setup.py

.PHONY: bandit
bandit: venv  # find common security issues in code
	$(VENV_BIN)/bandit -r ./aioapp_redis ./examples setup.py

.PHONY: mypy
mypy: venv ## lint
	$(VENV_BIN)/mypy aioapp_redis examples setup.py --ignore-missing-imports
...
coverage-quiet: venv ## make coverage report
		$(VENV_BIN)/coverage run --source aioapp_redis -m pytest
...
docs-quiet: venv ## make documentation
	rm -f docs/aioapp_redis.rst
	rm -f docs/modules.rst
	.venv/bin/sphinx-apidoc -o docs/ aioapp_redis
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
...
fast-test: venv ## fast-test
	pytest -s -v --redis-addr=redis://127.0.0.1:19831/ tests

.PHONY: fast-coverage
fast-coverage: venv ## make coverage report and open it in browser
		$(VENV_BIN)/coverage run --source aioapp_redis -m pytest tests -v --redis-addr=redis://127.0.0.1:19831/ tests
		$(VENV_BIN)/coverage report -m
		$(VENV_BIN)/coverage html
		$(BROWSER) htmlcov/index.html
